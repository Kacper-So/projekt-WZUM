...
with open('dataset3.csv', 'r') as f:
    ...
    for row in reader:
        dataset.append(row)
with open('Kacper_Sobczak_dataset_2.csv', 'r') as f:
    reader = csv.DictReader(f)
    for row in reader:
        dataset.append(row)
with open('Kacper_Sobczak_dataset.csv', 'r') as f:
    reader = csv.DictReader(f)
    for row in reader:
        dataset.append(row)
...
with open('model.pkl', 'wb') as plik:
    pickle.dump(clf, plik)
...
plot_confusion_matrix(confusion, ['g','o','p','a','b','c','d','e','f','h','i','k','l','m','n','q','r','s','t','u','v','w','x','y'], normalize=False)